...
%
Using the signal propagation models described in Chapter \ref{chap:RSP} in combination with the particle filter from chapter \ref{chap:PF} provides a framework for indoor positioning. This framework can then be used to perform position estimation using RSSI measurements from multiple WiFi APs. 
...
\end{equation}
%
This allows the noise to be low when the distance is short and large when the distance is large and the probability for obstructions is fairly large.
%
...
\item[Case 1:] \hfill \\
%
Measurements of the RSSI is taken at equally spaced points with $0.5$ meter spacing over a predetermined path. Only measurements from predetermined APs with known positions are taken. The measurements are taken while the measuring phone is at rest at each point.  
%
\item[Case 2:] \hfill \\
%
...
\emph{Case 2} provides a more natural use-case, where the user is moving while trying to find his position. This case lacks the ability for error analysis, as the measurements are taken continuously over an interval and only the estimated trajectory can be compared to the true one.  

For different tests, different paths and a different setup of APs is used. Further, all tests are performed in the same open office environment, but differences between the exact environments for each test exist. 
%
\section{Results}
%
...
\end{figure}
%
The methodology from \emph{case 1} was used during this test and measurements from seven APs along a path of aproximately one hundred meters. The locations of the APs together with the path taken may be found in Figure \ref{env_one}.
%
...
%
As stated before a quaternion, $q$, is a set of numbers, extending the complex number, defined as
%
\begin{equation}
