...
\caption{Absolute position error for the LS (\emph{red}) and PF (\emph{blue}) estimators for environment \emph{one}.}\label{ls_error_medium}
\end{figure}
...
For environment \emph{two} the LS estimation perform better compared to the PF, see the errors in figure \ref{ls_error_medium}. 

Medium path PF mean 3.4 LS mean 5.1
%
...
\caption{Figure text}\label{true_vs_est_env_2_05}
\end{figure}
%
\begin{figure}[!hbt]
\include{graphics}
...
%
The value of $C$ is specific for each frequency on each AP. Hence, the algorithm should store one value of $C$ for each frequency on each AP. The parameter $\alpha$ can be treated in different ways. One is to let alpha be the same for the whole building. This is suitable if the environment in the building is rather homogeneous, i.e. the density of obstacles such as walls furniture is relatively constant. Another alternative is to let $\alpha$ depend on the environment between the phone and each AP. 
%
