...
<<<<<<< HEAD
\title{Advantages of Using Simultaneous Measurements of 2.4 and 5 GHz Wi_Fi Signals for Indoor Positioning}%Temporary
=======
\title{Advantages of Using Simultaneous Measurements of 2.4 and 5 GHz Wi-Fi Signals for Indoor Positioning}%Temporary
>>>>>>> 9ba844e86285f3fb92d087bb434df350c3861019
\year{2014}
\month{January}
...
A typical choice of $n$ for indoor conditions is 2 or slightly less, to account for the line-of-sight case when $d$ is small, see table \ref{table:path_loss_n}. The range of $\alpha$ is around $[0.3,1.5]$ depending on the building.
...
For \ref{equation:fixed_alpha} the predicted signal strength for $n=2$, $\alpha=0.9$ and $d_{\text{max}}=20$ is shown together with measurements in figure \ref{fixed_alpha}. 
 %
 \begin{figure}[!hbt]
\include{graphics}
...
In this chapter the non-linear filtering problem will be explained along with its solution using the particle filter. Some computational aspects will be investigated and the particle filters uses in positioning briefly discussed. The chapter ends with a section about other estimation techniques where most focus are placed on the \emph{least squares estimation}.
%
...
%
The method \emph{least squares} is a common estimation technique for overdetermined systems, and is base on minimizing the error between a observed value and a modeled one. 
...
\end{equation}
%
given $n>m$. In the case of $f_i(x_k)$ being linear for all $i$, the algorithm is called the \emph{linear least squares} and $S$, in this case, has only one global minima due to convexity. Also, a analytic solution is always present.

Generalizing the linear least squares estimation to non-linear models is fairly simple. The sum in \ref{ls_sum} is still to be minimized, however in the non-linear case a closed-form analytical solution can not be found. Instead the minimum is found by numerical approximations. As $S$ in the non-linear case is not, in general, convex, there exists multiple local solutions to the minimization problem. This requires a guess of the approximate states to ''guide'' the algorithm to the correct solution, and if the guess is to far from the global minima, a local minima could be found instead.     

The non-linear least squares can be used to estimate a position in two dimensions from the models from chapter \ref{chap:RSP} together with measurements of signal strengths from at least three access points. 
%
...
%
The process noise is, in this case, rather straight forward to choose, as it represents the spread of particles in between measurements. A normal distribution around a circle with radius $r$ is a appropriate choice,
%
\begin{eqnarray}
...
A fair estimation is that a human walking travels at between 1 and 3 meters per second and given a interval $\Delta t$ between measurements $r = 2*\Delta t$ and $\sigma = 1*\Delta t$. Thus,
%
\begin{eqnarray}
...
\item[Case 1:] \hfill \\
%
Measurements of the RSSI is taken at equally spaced points with $0.5$ meter spacing over a predetermined path. Only measurements from predetermined APs with known positions is taken. The measurements are taken while the measuring phone is at rest at each point.  
%
\item[Case 2:] \hfill \\
%
Measurements of the RSSI is taken continuously while the user is walking a predetermined path at a relatively constant speed. The time between each measurement is approximately 4 seconds. Only measurements predefined APs with known positions are used.  
%
\end{description}
%
\emph{Case 1} allows the analysis of error between true position and estimated position as the measurements are taken at a fixed point. Further, the distance between measurements can be changed in increments of 0.5 meters by only using a subset of the available data. 

\emph{Case 2} provides a more natural use-case, where the user is moving while trying to find his position. This case lacks the ability for error analysis, as the measurements are taken continuously over a interval and only the estimated trajectory can be compared to the true one.  

For different test, different paths and a different setup of APs is used. Further, all test are performed in the same open office environment, but differences between the exact environments for each tests exists. 
%
\section{Results}
%
The results for the different tests performed is presented in the different sections below. 
%
...
\end{figure}
%
The methodology from \emph{case 1} was used during this test and measurements from seven APs along a path of around one hundred meters. The locations of the APs together with the path taken may be found in figure \ref{env_one}.
%
