...
Indoor positioning is a large research area, and various techniques have been proposed. They range from using only one portable device, to attaching several sensors on different parts of the user's body. 

There are numerous examples where WiFi APs with known locations are used to trilaterate the position of the user. The distance to each AP is often estimated using the signal strength, see e.g REF. This can also be done by measuring the time it takes for a signal to travel between the device and an AP, kanske see REF. 

Another method is \emph{pedestrian dead reckoning} (PDR), where sensors such as accelerometer and gyroscope are used to determine the motion of the user. An example of this is presented in \cite{exjobb}. Here, the accelerometer was used for counting steps, and the gyroscope was used to determine the change of heading. The step length, and the initial position and heading, were assumed to be known.

PDR has also been combined with e.g. WiFi signals into sensor fused approaches. 
