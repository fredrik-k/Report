...
The main strategy for detecting the attenuation is to note when the RSSI drops. However, moving away from the AP:s, or walking behind a wall, may lower the RSSI in a similar way. In order to distinguish the signal attenuation by the user only, the algorithm focuses on the six largest RSSI-values. If the average of these drops more than 6 dB, this indicates a block.
